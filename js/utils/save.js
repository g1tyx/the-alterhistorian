...
function getStartLayerData(layer) {
	...
	layerdata.processes = getStartProcesses(layer);
	layerdata.prevTab = "";
...
function getStartProcesses(layer) {
	let data = {};
	if (!layers[layer].processes) return data;

	for (let i in layers[layer].processes) {
		...
		for (let j in layers[layer].processes[i].requires) {
			data[i].sliders[j + '_req'] = layers[layer].processes[i].requires[j].sliderDefault || "0.5";
		}
		for (let j in layers[layer].processes[i].produces) {
			data[i].sliders[j + '_pro'] = layers[layer].processes[i].produces[j].sliderDefault || "0.5";
		}
	}
	return data;
}
...
function load() {
	...
	updateTemp();
	updateTemp();
	updateProcessActiveTemp();
	setupProcessActiveTemp();
	updateTabFormats();
	loadVue();
	retrieveCanvasData();
	loadEls();
