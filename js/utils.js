...
function canAffordPurchase(layer, thing, cost) {
	if (cost instanceof Decimal && decimalZero.eq(cost)) return true;
	if (isPlainObject(cost)) {
		for (let i in cost) {
			if (player[i].points.lt(cost[i])) return false;
		}
		return true;
	}
...
function buyUpg(layer, id) {
	...
	if (canAffordUpgrade(layer, id) === false) return;
...
function toNumber(x) {
	if (x.m !== undefined) return x.toNumber();
...
function objReducer(obj, func, startValue) {
	for (let i in obj) {
		startValue = func(startValue, obj[i], i);
	}
	return startValue;
}