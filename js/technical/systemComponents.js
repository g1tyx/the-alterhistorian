var systemComponents = {
			...
			<div class="upgRow">
				<div v-for="tab in Object.keys(data)">
					...
					v-bind:style="[{'border-color': tmp[layer].color}, (subtabShouldNotify(layer, name, tab) ? {'box-shadow': 'var(--hqProperty2a), 0 0 20px '  + (data[tab].glowColor || defaultGlow)} : {}), tmp[layer].componentStyles['tab-button'], data[tab].buttonStyle]"
						v-on:click="function(){if (data[tab].onClick) data[tab].onClick(); player.subtabs[layer][name] = tab; updateTabFormats(); needCanvasUpdate = true;}">{{data[tab].title || tab}}</button>
				</div>
			</div>
		`
	},
	...
	'tree-node': {
		...
		<button v-if="nodeShown(layer)"
			...
			v-bind:class="{
				...
				locked: tmp[layer].onClick ? !(player[layer].unlocked || tmp[layer].canReset) : !(tmp[layer].canClick),
	...
	'overlay-head': {
		...
		<br>
		<h2 id="points">The first Alterhistorian</h2>
		<br><br>
		<span class="overlayThing">
			<span v-if="player.secondary.unlocked">Secondary (II)</span>
			<span v-else>Origin (0, 0)</span>
		</span>
		<br>
	...
	'info-tab': {
        ...
        <a href="https://nagshell.github.io/elemental-inception-incremental/Historian/Historian.html" target="_blank">The First Alkahistorian</a> by Nagshell
		<br><br>
	...
	'toggle-switch': {
		...
		template: `<label class="switch">
			<input type="checkbox" v-model="object[property]">
